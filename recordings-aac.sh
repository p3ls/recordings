...
while read i ; do
 ...
 if [ "$j" = "stream_dir" ] ; then
  stream_dir=$(echo $i|cut -d'=' -f2)
 fi


 if [ "$j" = "cover_dir" ] ; then
  cover_dir=$(echo $i|cut -d'=' -f2)
 fi


 if [ "$j" = "log_dir" ] ; then
  log_dir=$(echo $i|cut -d'=' -f2)
 fi
 ...
 if [ "$j" = "cover_path" ] ; then
  cover_path=$(echo $i|cut -d'=' -f2)
 fi
done < $1
...
u='UTF-8'
# 10 = debug 
ed=$(($length_in_minutes*60)) # expected duration in seconds
...
for i in 1 2 3 ; do
 ...
 echo "expected length         : $length_in_minutes min"
...
streamripper $stream_url -d $stream_dir -a $filename2 -A -l $ed --codeset-filesys=UTF-8 --codeset-id3=ISO-8859-15 --codeset-metadata=ISO-8859-15 --codeset-relay=ISO-8859-15 # --codeset-metadata=$u --codeset-filesys=$u --codeset-id3=$u
 ...
 else if [ "$stream_format" = "mp3" ] ; then
